...
module.exports = function(passport){
	...
	passport.use('local-signup', new LocalStrategy({
		//by default local strateg uses username and password, we will ovverride with email
	...
	function(req, email, password, done) {
		...
		process.nextTick(function(){
			...
			User.findOne({'local.email' : email}, function(err, user){
				// if there are any erros then return the errors
				if(err)
					return done(err);
				...
				if(user) {
					...
					//set up user's local credentials
	...
	passport.use('local-login', new LocalStrategy({
		...
		usernameField : 'email',
		passwordField : 'password',
		passReqToCallback : true // allow us to pass back the entire reuest to callback
	},
	function(req, email, password, done){
		...
		User.findOne({'local.email' : email}, function(err, user){
			...
			// if the user is found but the passord is wrong
